...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        // Checks for initial selection or when the CPU is empty, selecting the correct process
        if (p == NULL) {
            // Lock readyQ before inspecting or modifying it — another CPU
            // thread (or main inserting a new arrival) could touch it right now.
            pthread_mutex_lock(&(svars->readyQLock));
        ...
        // Checks to see if a process in the ReadyQ has a shorter remaining burst time left
            ...
            // shortest time remaining process to run and removes it from the ReadyQ
...
void* PPcpu(void* param) {
    ...
    while (1) {
        ...
        // ── Selection (each time) ───────────────────────────────────
        // PP is preemptive: replacement can occur after each time tick cycle.
        // Preemption occurs when a higher priority job arrives in the queue.
        ...
        // Checks to see if a process in the ReadyQ has a higher priority
            ...
            // Puts the current process back into the ReadyQ while grabbing to the current
            // highest-priority process to run and removes it from the ReadyQ
            qInsert(&(svars->readyQ), p);
            p = qRemove(&(svars->readyQ), qPriority(&(svars->readyQ)));
